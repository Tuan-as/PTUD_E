// CHỨC NĂNG: LOGIC XỬ LÝ TRANG DANH SÁCH SỰ KIỆN

const EVENTS_PER_PAGE = 6; 
// đường dẫn api tính từ thư mục store_pages
const API_URL = 'fetchdata/fetch_events.php';  

// lấy các phần tử dom cần thiết
const eventsGrid = document.getElementById('events-grid'); 
const filterBtns = document.querySelectorAll('.filter-btn');
const paginationBar = document.getElementById('pagination-bar');

// biến trạng thái toàn cục
let allEventsData = [];
let currentStatus = 'all'; 
let currentPage = 1;

// CHỨC NĂNG: TẢI DỮ LIỆU SỰ KIỆN TỪ SERVER
async function fetchEventsData() {
    try {
        const response = await fetch(API_URL);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        
        const data = await response.json(); 
        
        if (Array.isArray(data) && data.length > 0) {
            allEventsData = data;
        } else {
             eventsGrid.innerHTML = '<p style="text-align: center; color: red;">Không có sự kiện nào được tìm thấy.</p>';
             return;
        }

        // hiển thị dữ liệu sau khi tải xong
        renderEvents(currentStatus, currentPage);

    } catch (error) {
        console.error("Lỗi khi tải dữ liệu Sự kiện:", error);
        eventsGrid.innerHTML = '<p style="text-align: center; color: red;">Lỗi kết nối.</p>';
    }
}

// CHỨC NĂNG: TẠO MÃ HTML CHO MỘT THẺ SỰ KIỆN
function createEventCardHTML(event) {
    const statusClass = `status-${event.status_slug}`;
    
    // đường dẫn trang chi tiết (nằm cùng thư mục store_pages)
    const detailUrl = `CC_SuKien_ChiTiet.php?id=${event.id}`;
    
    // đường dẫn ảnh (đi ra root rồi vào img_vid)
    const imageUrl = `../img_vid/${event.image_url.split('/').pop()}`;

    return `
        <a href="${detailUrl}" class="event-card"> 
            <div class="event-image-wrapper">
                <img src="${imageUrl}" alt="${event.title}" class="event-image">
            </div>
            <div class="event-content">
                <span class="event-status ${statusClass}">${event.status_label}</span>
                <h2 class="event-title">${event.title}</h2>
                <p class="event-description">${event.description.substring(0, 100)}...</p>
                <div class="event-meta">
                    <p>Thời gian: ${event.date_range}</p>
                    <p>Địa điểm: ${event.location}</p>
                </div>
            </div>
        </a>
    `;
}

// CHỨC NĂNG: LỌC, PHÂN TRANG VÀ HIỂN THỊ SỰ KIỆN
function renderEvents(status, page) {
    // lọc sự kiện theo trạng thái
    const filteredEvents = allEventsData.filter(event => 
        status === 'all' || event.status_slug === status
    );
    
    // tính toán chỉ số phân trang
    const totalPages = Math.ceil(filteredEvents.length / EVENTS_PER_PAGE);
    const startIndex = (page - 1) * EVENTS_PER_PAGE;
    const endIndex = startIndex + EVENTS_PER_PAGE;
    const eventsToDisplay = filteredEvents.slice(startIndex, endIndex);

    // chèn html vào lưới
    eventsGrid.innerHTML = eventsToDisplay.map(createEventCardHTML).join('');

    // hiển thị thanh phân trang
    renderPagination(totalPages, page);
}

// CHỨC NĂNG: XỬ LÝ KHI NGƯỜI DÙNG BẤM NÚT LỌC
function handleStatusChange(newStatus) {
    currentStatus = newStatus;
    currentPage = 1; 

    // cập nhật giao diện nút active
    filterBtns.forEach(btn => {
        btn.classList.remove('active');
        if (btn.dataset.status === newStatus) {
            btn.classList.add('active');
        }
    });

    renderEvents(newStatus, currentPage);
}

// CHỨC NĂNG: TẠO CÁC NÚT TRÊN THANH PHÂN TRANG
function renderPagination(totalPages, currentPage) {
    paginationBar.innerHTML = '';
    if (totalPages > 1) {
        // nút lùi (prev)
        const prevBtn = document.createElement('button');
        prevBtn.textContent = '<';
        prevBtn.className = 'page-btn';
        prevBtn.disabled = currentPage === 1;
        prevBtn.onclick = () => handlePageChange(currentPage - 1);
        paginationBar.appendChild(prevBtn);

        // các nút số trang
        for (let i = 1; i <= totalPages; i++) {
            const btn = document.createElement('button');
            btn.textContent = i;
            btn.className = 'page-btn';
            if (i === currentPage) {
                btn.classList.add('active');
            }
            btn.onclick = () => handlePageChange(i);
            paginationBar.appendChild(btn);
        }

        // nút tiến (next)
        const nextBtn = document.createElement('button');
        nextBtn.textContent = '>';
        nextBtn.className = 'page-btn';
        nextBtn.disabled = currentPage === totalPages;
        nextBtn.onclick = () => handlePageChange(currentPage + 1);
        paginationBar.appendChild(nextBtn);
    }
}

// CHỨC NĂNG: XỬ LÝ KHI CHUYỂN TRANG
function handlePageChange(newPage) {
    currentPage = newPage;
    renderEvents(currentStatus, currentPage);
    // cuộn lên đầu trang mượt mà
    window.scrollTo({ top: 0, behavior: 'smooth' }); 
}

// CHỨC NĂNG: GẮN SỰ KIỆN CLICK CHO CÁC NÚT LỌC
filterBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        handleStatusChange(btn.dataset.status);
    });
});

// CHỨC NĂNG: KHỞI TẠO KHI TRANG TẢI XONG
document.addEventListener('DOMContentLoaded', () => {
    fetchEventsData(); 
});